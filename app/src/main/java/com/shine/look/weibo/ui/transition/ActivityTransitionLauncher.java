...
public class ActivityTransitionLauncher {
    ...
    public void launch(Intent intent) {
        ...
        if (fromView instanceof ImageView) {
            Rect rect = ((ImageView) fromView).getDrawable().getBounds();
            intent.putExtra(appId + EXTRA_IMAGE_WIDTH, rect.right).
                    putExtra(appId + EXTRA_IMAGE_HEIGHT, rect.bottom);
        } else {
            intent.putExtra(appId + EXTRA_IMAGE_WIDTH, fromView.getMeasuredWidth()).
